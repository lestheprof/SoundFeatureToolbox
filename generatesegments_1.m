function [ noofexperiments ] = generatesegments_1( stimulibasedir, experimentname,dirname, input_filelist,...
...
% started 27 Feb 2014.
%
% This version allows for having a number of phonemes in each segment. But
% it also tries to avoid in cluding aphoneme that only just has the segment
% within it. In addition, segmentphonemes is now a cell array, allowing the
% use of unique on it, later. Note the use of curly brackets for accessing
% strings in cells as strings, and not as cells. 
%
% preparatory stuff
...
fileset = 'fileset' ;
timit = 0 ; % set to one if this is from the TIMIT dataset, so that .phn files can be used as well.
minlimen = 0.005 ; % 5ms minimum for phoneme to be included in a segment
i=1 ;
...
if timit
    segmentphoneme = cell([noofexperiments * maxsegsperfile 1]) ; % use for phoneme (TIMIT only)
    segmentphoneme(:) = {''} ;
end
for j=1:noofexperiments
    ...
    if (timit)
        timitphons = fopen([stimulibasedir experimentname  dirname '/' startpart '.PHN']) ;
    end
    ...
    if timit
        tphon = textscan(timitphons, '%d%d%s\n') ;
        % x1 contains a cell array, with the 1st element being the start
        % time of a section, the 2nd element the end time thereof, and the
        % 3rd element being the code for the phoneme in there. There's one
        % element in the array for each section in that timit signal
        % convert times to seconds
        tphon{1} = double(tphon{1})/ANSigInfo.ANParams.Fs ;
        tphon{2} = double(tphon{2})/ANSigInfo.ANParams.Fs ;
        ...
        if (((OnsetInfo.intervals(onsetno,2) - OnsetInfo.intervals(onsetno,1)) > minintervallength) ... % length
            ...
            if (vectstart >=1)
                ...
                if ((vectstart + vectlength-1) <= maxlength)
                    ...
                    if timit
                        % store the phoneme value for this segment
                        x1 = find((tphon{2} - segmentstart(meansIntervalno)) >= minlimen, 1) ; % which section is the interval start in
                        x2 = find((tphon{1} - (segmentstart(meansIntervalno) + seglength)) >= minlimen, 1) ; % which section is the interval end after
                        x2 = x2 -1 ;
                        if (x2 == 0) % offset before official start of sound
                            segmentphoneme{meansIntervalno} = 'h#' ;
                        else
                            if (isempty(x2) || (x1 == x2)) % also in case segment goes off the end
                                segmentphoneme{meansIntervalno} = tphon{3}{x1} ;
                            else
                                % x2 > x1: if x2 = x1 + 1 we need to look at which section
                                
                                % x2 > x1 + 1
                                if x1 == 0
                                    error(['generatesegments: error: x1 = 0, x2 = ' num2str(x2)]) ;
                                end
                                % concatenate the phonemes, with a ,
                                segmentphoneme{meansIntervalno} = tphon{3}{x1} ;
                                
                                % separator
                                for phno = x1+1:x2
                                    segmentphoneme{meansIntervalno} = strcat(segmentphoneme{meansIntervalno}, ',', tphon{3}{phno}) ;
                                end
                            end
                            %
...
if (timit)
    segmentphoneme = segmentphoneme(1:meansIntervalno-1) ;
end
...
if (timit) % include segmentphoneme
    save([stimulibasedir  experimentname  dirname '/' OutputDir '/' fileset '_vectors' '.mat'],...
        'meansoutputarray', 'meansonsetarray', 'meansgaborarray','segmentid','segmentstart','segmentphoneme', 'vectors', 'ANParams', ...
        'onsetparams', 'mlmparams', 'mloparams', 'gaborparams' ) ;
else % no segmentphoneme
    save([stimulibasedir  experimentname  dirname '/' OutputDir '/' fileset '_vectors' '.mat'],...
