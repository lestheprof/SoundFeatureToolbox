function runothers_fn(otherlocation, filelist, ANSigDir, AMMeanPlain250, onsetSigdir, onsetMeans250, gaborparamsall,outputdir, varargin)
...
create_AN_files(otherlocation, '','', filelist, store_bmSig, bmSigdir, ANSigDir) ;
generateonsetspikes2_mono(otherlocation, '','', filelist, ANSigDir, onsetSigdir, display) ;


n_run = generatemeanplain_mono(otherlocation, '', '', filelist, ANSigDir, AMMeanPlain250, resamplerate, 0) ;
mo = generatemeanlevelonset_mono(otherlocation, '', '', filelist, onsetSigdir, onsetMeans250, resamplerate, 0) ;
% gaborparams.bandwidth = 0.7 ;
% gaborparams.gamma = 4 ;
% gaborparams.theta = pi/2 ;
% gaborparams.lambda = 18 ;
gaborparams = gaborparamsall{1} ;
n_gaborrun = generategaboroutputs_mono(otherlocation, '', '', filelist, ANSigDir, gabordirs{1}, gaborparams,resamplerate, 0) ;
gaborparamsb = gaborparamsall{2} ;
% gaborparamsb.lambda = 10 ;
n_gaborrunb = generategaboroutputs_mono(otherlocation, '', '', filelist, ANSigDir, gabordirs{2}, gaborparamsb,resamplerate, 0) ;

nm = generatesegments_1(otherlocation, '', '',filelist_male, onsetSigdir,  AMMeanPlain250, onsetMeans250, gabordirs, outputdir, ...
    'minimumspikes', 10, 'fileset', 'expt_male', 'pretime', pretime, 'timit', timit, 'seglength', seglength) ;
disp(['male 250 resample done ' num2str(nm)]) ;
nf = generatesegments_1(otherlocation, '', '',filelist_female, onsetSigdir,  AMMeanPlain250, onsetMeans250, gabordirs, outputdir, ...
...
m1 = load([otherlocation '/' outputdir '/expt_male_vectors.mat']) ;
...
f1 = load([otherlocation '/' outputdir '/expt_female_vectors.mat']) ;
...
save([otherlocation '/' outputdir '/expt_all_PCA'], 'pcas_female', 'pcas_male',  'f1', 'm1') ;
save([otherlocation '/' outputdir '/expt_all_ICA20'], 'icas1d_f', 'icas2d_f', 'proj_f', 'f1', 'm1', ...
    'icas1d_m', 'icas2d_m', 'proj_m') ;

% create the clusters (clustercreate)
for components = 4:1:16
for clusts = 3:1:24
    clusterpcaica_fn(otherlocation, outputdir, ['clustertest_C' num2str(components) '_clusts' num2str(clusts)], 'pcastouse', components, ...
        'icastouse', components, 'clusters_pca', clusts, 'clusters_ica', clusts) ;
    [f1{(components-3), (clusts-2)}, m1{(components-3), (clusts-2)}] = processclusters_fn(otherlocation, outputdir, 'expt_all_PCA.mat', ...
        ['clustertest_C' num2str(components) '_clusts' num2str(clusts)], 'const_phonfraction', 1.0) ;
end
end
save([otherlocation '/' outputdir '/clusterresults14032104.mat'], 'f1', 'm1') ;

% analyse clusters
cl2 = load([otherlocation '/' outputdir '/clusterresults14032104.mat']) ;
...
save([otherlocation '/' outputdir '/clusteranalysis14032104.mat'], 'NMIf_p', 'QFf_p', 'NMIm_p', 'QFm_p', 'NMIm_i', 'QFm_i', 'NMIf_i', 'QFf_i') ;  ;
end
