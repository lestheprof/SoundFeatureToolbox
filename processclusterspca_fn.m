function [fem, male] = processclusterspca_fn(timitlocn, vdirectory, segmentsfile, clustersfile , varargin)
...
while(i<=size(varargin,2))
    switch lower(varargin{i})
        ...
        otherwise
            error('processclusterspca_fn: Unknown argument %s given',varargin{i});
    end
    i=i+1;
end
%
...
clustdata_m = load([timitlocn '/' vdirectory '/' clustersfile '_pca_m']) ;
clustdata_f = load([timitlocn '/' vdirectory '/' clustersfile '_pca_f']) ;
