function [totalfiretimes, wide_times] = onsetside_zc9_mono(anspikes, onsetparams)
...
% Modified to use more compact AN representation LSS 31 may 2013.
% Modified to use the level of the AN spike, as the presynaptic input:
...
wttoonset_wide = onsetparams.onsetcellwt_singlewide ; % weight to onset cell: different for single unified onsets
...
% process the new AN.signal structure to provide the firetimes array
% firetimes contains the spikes for all the channels at one specific
% sensitivity level, ordered by time
% Hom mny spikes are there?
totalspikes = 0 ;
spikesperband = zeros([1 anspikes.channels]) ;
for b = 1:anspikes.channels
    spikesperband(b) = size(anspikes.signal{b}, 2) ;
    totalspikes = totalspikes + size(anspikes.signal{b}, 2) ;
end
% initialise array for holding all the spikes 
totalfiretimes = zeros([3 totalspikes]) ;
% fill the array: channel, time (in samples), maximal sensitivity level
startpoint = 1 ;
for b = 1:anspikes.channels
    totalfiretimes(1,startpoint:startpoint + spikesperband(b) - 1) = b ;
    totalfiretimes(2:3,startpoint:startpoint + spikesperband(b) - 1) = anspikes.signal{b} ;
    startpoint = startpoint + spikesperband(b) ;
end
% totalfiretimes contains all the spikes in all sensitivity levels and all
% channels. 
% sort into time order
totalfiretimes = sortrows(totalfiretimes', [2 1])' ;
% convert tims to seconds
totalfiretimes(2,:) = totalfiretimes(2,:)/fs ;
% process all at once (modded 5 4 2016)
% for iterno = 1:iterations

    % firetimes = totalfiretimes(:, totalfiretimes(3,:) >=iterno )';
...
%     end
    ...
    activity_wide = depsynsimple2(totalfiretimes', fs, numchans, datalength, ...
    ...
    wide_times.list = iandfneurons(activity_widenew * wttoonset_wide, fs, ...
