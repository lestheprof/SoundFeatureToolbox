...
if timit
segmentphoneme = cell([1 noofexperiments * maxsegsperfile]) ; % use for phoneme (TIMIT only)
end
            ...
            if (vectstart >=1)
                ...
                if ((vectstart + vectlength-1) <= maxlength)
                    ...
                    if timit
                        ...
                        x1 = find((tphon{2} - segmentstart(meansIntervalno)) >= 0, 1) ; % which section is the interval start in
                        x2 = find((tphon{1} - (segmentstart(meansIntervalno) + seglength)) >= 0, 1) ; % which section is the interval end after
                        x2 = x2 -1 ;
                        if (x2 == 0) % offset before official start of sound
                            ...
                            if (isempty(x2) || (x1 == x2)) % also in case segment goes off the end
                                ...
                                % contains more of the interval
                                if (x2 == (x1 + 1))
                                    if (segmentstart(meansIntervalno) - tphon{1}(x1)) >= ...
                                            (segmentstart(meansIntervalno) + seglength) - tphon{1}(x2)
                                        segmentphoneme(meansIntervalno) = tphon{3}(x1) ;
                                    else segmentphoneme(meansIntervalno) = tphon{3}(x2) ;
                                    end
                                else % x2 > x1 + 1
                                    if x1 == 0 
                                        error(['generatesegments: error: x1 = 0, x2 = ' num2str(x2)]) ;
                                    end
                                    segmentphoneme(meansIntervalno) = tphon{3}(floor((x1 + x2 - 1) /2)) ; % use the middle one
                                end
                                %
                            end
                        end
