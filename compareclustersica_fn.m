function [normmutinf, qualityfigure] = compareclustersica_fn(f1)
...
normmutinf = zeros([numcomponents numclusters]) ;
mutinf = zeros([numcomponents numclusters]) ;
...
for compno = 1:numcomponents
    for numclust=1:numclusters
        ...
        for clno = 1:length(f1{compno, numclust}.icamodephonemes)
            totalphonemes = [totalphonemes; f1{compno, numclust}.icamodephonemes{clno}] ;
        end
        utotalphonemes = unique(totalphonemes) ;
        % put the icamodephonemes lists in order, putting the icaphonfreqclust in
        % order at the same time
        for clno = 1:length(f1{compno, numclust}.icamodephonemes)
            [sficamodeph{clno}, remap]= sort(f1{compno, numclust}.icamodephonemes{clno}) ;
            sficaphonfreq{clno} = f1{compno, numclust}.icaphonfreqclust{clno}(remap) ;
        end
        % create array to put occurrences into
        allphonemes = zeros([length(utotalphonemes) length(f1{compno, numclust}.icamodephonemes)]) ;
        ...
        qtemp = zeros([1 length(f1{compno, numclust}.icamodephonemes)]) ;
        for pno = 1:length(utotalphonemes)
        ...
        for clno = 1:length(f1{compno, numclust}.icamodephonemes)
            qualityfigure(compno, numclust) =  qualityfigure(compno, numclust) + ...
                qtemp(clno)/length(unique(f1{compno, numclust}.icamodephonemes{clno})) ;
        end
        qualityfigure(compno, numclust) = qualityfigure(compno, numclust)/numclusters ;
