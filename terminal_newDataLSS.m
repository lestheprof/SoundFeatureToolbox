...
% Location where data stored
 % dataPath = 'F:/f101m102/';
 %dataPath = 'F:/testallen2/';
% for LSS Mac
dataPath = '/Volumes/MacHD2/Researchshare/timit3/allenESNdata/f101m102/';

% File name to use for input data
outDataName = 'inputDataSet.mat' ;
% outDataName = 'female.mat';

datasetup = 0 ; % 1 to run setup, 0 if data already setup
runESN = 1 ;  %0; % 1 to run ESN, 0 to not run


%setup.mloDir = 'onsetMeans2'; % Onset dir to add in
%setup.mlaDir = 'AMMeanPlain'; % AN dir to add in
%setup.gabDirs = {'gabor25042014_1','gabor25042014_2','gabor25042014_3'};
...
if datasetup
    ...
    setup.useAn = 1;
    setup.useGabors = 0;
    setup.gender = 'mf'; % varargin parameter to determine if gender split.  Optional.  'mf' is for both genders
    formatEsnData(setup.useOnset,setup.useAn,setup.useGabors,dataPath,...
        setup.mloDir,setup.mlaDir,setup.gabDirs,'outname',outDataName, 'gender', setup.gender);
    
   % Original with no gender or varargin 
   % formatEsnData(setup.useOnset,setup.useAn,setup.useGabors,dataPath,...
...
if runESN
    ...
    temp1=load([dataPath,outDataName]);
