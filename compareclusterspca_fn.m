function [normmutinf, qualityfigure] = compareclusterspca_fn(f1)
...
for compno = 1:numcomponents
    for numclust=1:numclusters
        ...
        qtemp = zeros([1 length(f1{compno, numclust}.pcamodephonemes)]) ;
        for pno = 1:length(utotalphonemes)
        ...
        for clno = 1:length(f1{compno, numclust}.pcamodephonemes)
            qualityfigure(compno, numclust) =  qualityfigure(compno, numclust) + ...
                qtemp(clno)/length(unique(f1{compno, numclust}.pcamodephonemes{clno})) ;
        end
