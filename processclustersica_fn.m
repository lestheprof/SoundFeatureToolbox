...
clustdata_m = load([timitlocn '/' vdirectory '/' clustersfile '_ica_m']) ;
clustdata_f = load([timitlocn '/' vdirectory '/' clustersfile '_ica_f']) ;
...
for  i = 1:clustdata_m.icaparams.clusters_ica % for each cluster
    icaphonemes{i} = segdata.m1.segmentphoneme(clustdata_m.line1to8_km5_icam == i) ; % find the phonemes in this cluster
    % find the list of phonemes, and the index in that list of each
    [icaphonemesunique{i}, ~, icaphuid{i}] = unique(icaphonemes{i}) ;
