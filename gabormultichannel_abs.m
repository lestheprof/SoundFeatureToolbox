function [ finalsignal2d ] = gabormultichannel_abs(resamplerate, AN, gb5, type)
...
% 
% adds in absoulte value of Gabor patch
% 13 12 2013
lasttime = 0 ;
...
for ch = 1: AN.channels
    ...
    if (ch - halfheight <= 0) % won't fit, falls off bottom
        finalsignal2d(:, 1:ch+halfheight) = finalsignal2d(:, 1:ch+halfheight) + abs(gaborchan(:, 2 + (halfheight - ch):end)) ;
    end
    if (ch + halfheight > AN.channels) % won't fit, falls off top: will only do both if height of gabor > number of channels
        finalsignal2d(:, ch - halfheight :end) = finalsignal2d(:, ch - halfheight :end) + abs(gaborchan(:, 1:AN.channels - (ch - halfheight) + 1)) ;
    end
    if ((ch - halfheight > 0) && (ch + halfheight <= AN.channels)) % add the lot
        finalsignal2d(:, ch-halfheight:ch+halfheight) = finalsignal2d(:, ch-halfheight:ch+halfheight) + abs(gaborchan) ;
    end
end
