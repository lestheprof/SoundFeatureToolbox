function X = depsynsimple2(firetimes,sampleRate, numChannels, dataLength, gperms, alpha, beta)
...
%
% modified 5 April 2016 LSS to use the level of the AN spike
...
for stepno=2:1:dataLength
   ...
   if (firenumber <= lfiretimes)
       while (firetimes(firenumber,2) < endsteptime) 
           Z(firetimes(firenumber,1)) = g1 * firetimes(firenumber,3) ; % multiply by level inside firetimes?
