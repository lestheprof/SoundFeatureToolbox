function generateonsetspikes3_mono(stimulibasedir, experimentname,dirname, input_filelist, anSigDir, onsetSigDir, display, varargin)
...
while(i<=size(varargin,2))
    switch lower(varargin{i})
        ...
        otherwise
            error('generateonsetspikes3_mono: Unknown argument %s given',varargin{i});
    end
    i=i+2 ;
end
...
for j=1:noofexperiments
    ...
    [leftan,  leftonset_wide] = ...
        genonsetspikes3_mono(z1.AN, onset) ;
    ...
    [intervals, Nusedininteerval] = overallonset(leftonset_wide, noCochFilt, onset.sensitivitygap, onset.intervalgap,0) ;
    ...
    if (display==1)
        ...
        spikeraster(leftonset_wide.list,'Range',1:noCochFilt, 'EndTime',double(datalength)/fs, 'Xlabel', 'Left') ;
    ...
    if (store_onsetspikes)
        ...
        % added for compatibiity with AN spikes LSS 12 9 2016
        AN = ANparams ;
        onsetparams = onset ;
